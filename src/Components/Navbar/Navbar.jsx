  ...
  return (
    <nav className='flex flex-wrap justify-between md:items-center text-white px-10 pt-6 md:px-20'>
        ...
        <ul className={`${
            ...
            <a href="#Profiles">
            <li className='text-md transition-all duration-300 p-1 md:p-0'>Profiles</li>
            </a>
            <a href="#Projects">
        ...
        {showMenu ? (
            <RiMenu2Line size={30} className="md:hidden absolute right-10 top-6 transisition-all duration-300" onClick={()=>{openMenu(!menu); setShowmenu(!showMenu);}} />
        ) : (
            <RiCloseLine size={30} className="md:hidden absolute right-10 top-6 transisition-all duration-300"/>
        )}
    </nav>
  )
}
